...
interface StudentDao {
    ...
    @Query("""
       SELECT id, firstName, lastName, albumNumber, teacherId
       FROM ${DatabaseTableName.STUDENTS}
       WHERE teacherId = :currentUserId
    """)
    suspend fun getAllCurrentUserStudentsById(currentUserId: Long): List<Student>
    ...
    @Query("""
       ...
       LIMIT 1
    """)
    suspend fun getStudentDataById(studentId: Long): Student

    //Query for fetching current subject students:
    @Query("""
        SELECT s.id, s.firstName, s.lastName, s.albumNumber, s.teacherId
        ...
        """)
    suspend fun getSubjectStudentsBySubjectId(subjectId: Long): List<Student>
    ...
    @Query("""
        ...
        WHERE ss.subjectId <> :subjectId 
        """)
    suspend fun getNotSubjectStudentsBySubjectId(subjectId: Long): List<Student>
